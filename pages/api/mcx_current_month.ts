...
export default async function handler(
  ...
  try {
    ...
    // Process data to avoid duplicates (keep only the latest price for each day)
    ...
    snapshots.forEach(snapshot => {
      ...
      // Only keep the latest record for each day
      processedData.set(dateKey, {
        date: dateKey,
        value: parseFloat(snapshot.month1Price.toString()),
